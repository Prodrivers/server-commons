...
public class PartyCommands implements CommandExecutor {
	...
	private void partyList(final Player player) {
		...
		if(party != null) {
			...
			if(owner == null) {
				Main.logger.severe("Owner " + party.getOwnerUniqueId() + "  of party is null on party list command by " + player);
				return;
			}
			...
			for(final UUID p_ : party.getPlayers()) {
				...
				if(other == null) {
					Main.logger.severe("Party player " + p_ + "  of party is null on party list command by " + player);
				} else {
