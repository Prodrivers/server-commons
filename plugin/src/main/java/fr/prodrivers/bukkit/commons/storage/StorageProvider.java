...
public class StorageProvider {
	...
	public static void init() {
		try {
			if( Main.getConfiguration().storage_uri == null ) {
				Main.logger.info( "[ProdriversCommons] Storage provider is disabled, as no connection URI was provided." );
				return;
			}
			...
			if( uri.getDatabase() == null || uri.getDatabase().isEmpty() ) {
				Main.logger.warning( "[ProdriversCommons] Storage provider is not available: database name not provided in connection URI" );
				return;
			}
			...
			database = null;
			Main.logger.warning( "[ProdriversCommons] Storage provider is not available: " + ex.getLocalizedMessage() );
			Main.logger.warning( "[ProdriversCommons] Please check your storage connection URI and credentials." );
		}
	}
	...
	public static MongoCollection<Document> getPlayersCollection() {
		try {
			...
			Main.logger.severe( "[ProdriversCommons] Player collection retrieval went wrong: " + ex.getLocalizedMessage() );
		}
		return null;
	}
	...
	public static Document getPlayer( UUID playerUniqueId ) {
		...
		try {
			...
			Main.logger.severe( "[ProdriversCommons] Player retrieval in storage went wrong: " + ex.getLocalizedMessage() );
		}
	...
	public static Document createPlayer( UUID playerUniqueId ) {
		...
		try {
			...
			Main.logger.severe( "[ProdriversCommons] Player creation in storage went wrong: " + ex.getLocalizedMessage() );
		}
